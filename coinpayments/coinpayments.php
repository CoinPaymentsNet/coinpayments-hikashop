...
class plgHikashoppaymentCoinpayments extends hikashopPaymentPlugin
{
    ...
    public function getPaymentDefaultValues(&$element)
    {
        ...
        $element->payment_description = 'You can pay with Bitcoin or other cryptocurrencies via';
        ...
        foreach($methods as $method) {
            if ($method->payment_name == "CoinPayments") {
                foreach($usable_methods as $usable_method) {
                    if ($usable_method->payment_name == "CoinPayments") {
                        $method->payment_description = $usable_method->payment_description . '<a href="https://alpha.coinpayments.net/" target="_blank" title="CoinPayments.net">CoinPayments.net</a>';
                        $method->payment_description = str_replace("</p>", " ", $method->payment_description);
                        $method->payment_description .= "</p>";
                    }
                }
            }
        }
